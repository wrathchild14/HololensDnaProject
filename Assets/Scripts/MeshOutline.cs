...
/// <summary>
///     Component which can be used to render an outline around a mesh renderer. Enabling this component introduces an
///     additional render pass
///     of the object being outlined, but is designed to run performantly on mobile Mixed Reality devices and does not
///     utilize any post processes.
///     This behavior is designed to be used in conjunction with the MRTK/Standard shader. Limitations of this effect
///     include it not working well
///     on objects which are not watertight (or required to be two sided) and depth sorting issues can occur on overlapping
///     objects.
/// </summary>
...
public class MeshOutline : BaseMeshOutline
{
    ...
    private static int GetMinRenderQueue(Material[] materials)
    {
        ...
        if (min == int.MaxValue) min = (int)RenderQueue.Background;
    ...
    protected override void ApplyOutlineMaterial()
    {
        if (outlineMaterial != null && meshRenderer != null)
        {
            Debug.AssertFormat(outlineMaterial.IsKeywordEnabled(vertexExtrusionKeyword),
                "The material \"{0}\" does not have vertex extrusion enabled, no outline will be rendered.",
            ...
            {
                var meshSmoother = createdMeshSmoother == null
                    ? gameObject.GetComponent<MeshSmoother>()
                    : createdMeshSmoother;
